...
for i in `seq 3`;do for i in ${circle[@]};do echo -ne $TURQUOISE"\r$i";sleep 0.03;done;sleep 0.01;done;echo
if [ -f /usr/bin/notify-send ]; then
	...
	if [ $notify = 'y' ]; then
		...
		for i in `seq 3`;do for i in ${circle[@]};do echo -ne $TURQUOISE"\r$i";sleep 0.03;done;sleep 0.03;done;echo
	else
...
if [ -f /usr/bin/rsync ]; then
	...
	hash rsync
	for i in `seq 3`;do for i in ${circle[@]};do echo -ne $TURQUOISE"\r$i";sleep 0.01;done;sleep 0.03;done;echo
else
...
if [ -f $HOME/.HomeSync/homesync.txt ]; then
...
be prompted to set new path(s) to your backup device(s)."
	...
	for i in `seq 3`;do for i in ${circle[@]};do echo -ne $TURQUOISE"\r$i";sleep 0.03;done;sleep 0.03;done;echo
	echo "homesync updated on $(date +%A,+%m/%d/%Y) at $(date +%r)" >> $HOME/.HomeSync/homesync.txt
	echo -e "Loading. . . "
	...
	touch $HOME/.HomeSync/homesync.txt
...
time will be deleted, so you'll have to re-enter the path to any device you are going to continue using."
	echo -e "You'll need the EXACT path (/run/media/$HOME/device_Name)"
	...
	for pic in *.png;do sudo cp -f $pic /usr/share/icons;echo "Moving $pic to /usr/share/icons" ;done
	echo -e $WHITE"Currently mounted devices: $(df -hx tmpfs --output=target)"
	sudo python2 $HOME/.HomeSync/HomeSync/etc.py
fi
